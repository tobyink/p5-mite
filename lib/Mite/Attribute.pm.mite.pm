...
sub new {
    ...
    if ( exists($args->{q[default]}) ) { do { package Mite::Miteception; (do { package Mite::Miteception; !defined($args->{q[default]}) } or do { package Mite::Miteception; defined($args->{q[default]}) and do { ref(\$args->{q[default]}) eq 'SCALAR' or ref(\(my $val = $args->{q[default]})) eq 'SCALAR' } } or do { package Mite::Miteception; ref($args->{q[default]}) eq 'CODE' }) } or require Carp && Carp::croak(q[Type check failed in constructor: default should be Undef|Str|CodeRef]); $self->{q[default]} = $args->{q[default]};  }
...
*default = sub { @_ > 1 ? do { do { package Mite::Miteception; ((!defined($_[1])) or do { package Mite::Miteception; defined($_[1]) and do { ref(\$_[1]) eq 'SCALAR' or ref(\(my $val = $_[1])) eq 'SCALAR' } } or (ref($_[1]) eq 'CODE')) } or require Carp && Carp::croak(q[Type check failed in accessor: value should be Undef|Str|CodeRef]); $_[0]{q[default]} = $_[1]; $_[0]; } : ( $_[0]{q[default]} ) };
