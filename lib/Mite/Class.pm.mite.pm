...
sub new {
    ...
    if ( exists $args->{"extends"} ) { (do { package Mite::Miteception; ref($args->{"extends"}) eq 'ARRAY' } and do { my $ok = 1; for my $i (@{$args->{"extends"}}) { ($ok = 0, last) unless do { package Mite::Miteception; defined($i) and do { ref(\$i) eq 'SCALAR' or ref(\(my $val = $i)) eq 'SCALAR' } } }; $ok }) or Mite::Shim::croak( "Type check failed in constructor: %s should be %s", "extends", "ArrayRef[Str]" ); $self->{"extends"} = $args->{"extends"};  } else { my $value = do { my $default_value = do { my $method = $Mite::Class::__extends_DEFAULT__; $self->$method }; do { package Mite::Miteception; (ref($default_value) eq 'ARRAY') and do { my $ok = 1; for my $i (@{$default_value}) { ($ok = 0, last) unless do { package Mite::Miteception; defined($i) and do { ref(\$i) eq 'SCALAR' or ref(\(my $val = $i)) eq 'SCALAR' } } }; $ok } } or Mite::Shim::croak( "Type check failed in default: %s should be %s", "extends", "ArrayRef[Str]" ); $default_value }; $self->{"extends"} = $value;  } $self->_trigger_extends( $self->{"extends"} );
    if ( exists $args->{"parents"} ) { (do { package Mite::Miteception; ref($args->{"parents"}) eq 'ARRAY' } and do { my $ok = 1; for my $i (@{$args->{"parents"}}) { ($ok = 0, last) unless (do { use Scalar::Util (); Scalar::Util::blessed($i) and $i->isa(q[Mite::Class]) }) }; $ok }) or Mite::Shim::croak( "Type check failed in constructor: %s should be %s", "parents", "ArrayRef[InstanceOf[\"Mite::Class\"]]" ); $self->{"parents"} = $args->{"parents"};  };

    # Enforce strict constructor
    my @unknown = grep not( /\A(?:attributes|extends|imported_functions|name|parents|r(?:equired_methods|oles)|s(?:him_name|ource))\z/ ), keys %{$args}; @unknown and Mite::Shim::croak( "Unexpected keys in constructor: " . join( q[, ], sort @unknown ) );
...
sub superclasses { @_ > 1 ? do { my @oldvalue; @oldvalue = $_[0]{"extends"} if exists $_[0]{"extends"}; do { package Mite::Miteception; (ref($_[1]) eq 'ARRAY') and do { my $ok = 1; for my $i (@{$_[1]}) { ($ok = 0, last) unless do { package Mite::Miteception; defined($i) and do { ref(\$i) eq 'SCALAR' or ref(\(my $val = $i)) eq 'SCALAR' } } }; $ok } } or Mite::Shim::croak( "Type check failed in %s: value should be %s", "accessor", "ArrayRef[Str]" ); $_[0]{"extends"} = $_[1]; $_[0]->_trigger_extends( $_[0]{"extends"}, @oldvalue ); $_[0]; } : ( $_[0]{"extends"} ) }

# Accessors for parents
sub _clear_parents { delete $_[0]{"parents"}; $_[0]; }
sub parents { @_ > 1 ? Mite::Shim::croak( "parents is a read-only attribute of @{[ref $_[0]]}" ) : ( exists($_[0]{"parents"}) ? $_[0]{"parents"} : ( $_[0]{"parents"} = do { my $default_value = $_[0]->_build_parents; do { package Mite::Miteception; (ref($default_value) eq 'ARRAY') and do { my $ok = 1; for my $i (@{$default_value}) { ($ok = 0, last) unless (do { use Scalar::Util (); Scalar::Util::blessed($i) and $i->isa(q[Mite::Class]) }) }; $ok } } or Mite::Shim::croak( "Type check failed in default: %s should be %s", "parents", "ArrayRef[InstanceOf[\"Mite::Class\"]]" ); $default_value } ) ) }
