...
arguments. If the attribute C<< is => "rw" >>, then these aliases can
be used as an alias for the accessor; otherwise they are aliases for
the reader.
...
This feature is not supported by Moose, but is analagous to
L<MooseX::Aliases>.
