...
sub new {
    ...
    # has declaration, file lib/MooseInteg/BaseClass.pm, line 6
    ...
    # has declaration, file lib/MooseInteg/BaseClass.pm, line 11
    ...
    # Attribute xyzzy (type: Int)
    # has declaration, file lib/MooseInteg/BaseClassRole.pm, line 5
    if ( exists $args->{"xyzzy"} ) { (do { my $tmp = $args->{"xyzzy"}; defined($tmp) and !ref($tmp) and $tmp =~ /\A-?[0-9]+\z/ }) or MooseInteg::Mite::croak "Type check failed in constructor: %s should be %s", "xyzzy", "Int"; $self->{"xyzzy"} = $args->{"xyzzy"}; } ;
    ...
    my @unknown = grep not( /\A(?:foo|hashy|xyzzy)\z/ ), keys %{$args}; @unknown and MooseInteg::Mite::croak( "Unexpected keys in constructor: " . join( q[, ], sort @unknown ) );
...
# Accessors for xyzzy
# has declaration, file lib/MooseInteg/BaseClassRole.pm, line 5
sub xyzzy { @_ > 1 ? do { (do { my $tmp = $_[1]; defined($tmp) and !ref($tmp) and $tmp =~ /\A-?[0-9]+\z/ }) or MooseInteg::Mite::croak( "Type check failed in %s: value should be %s", "accessor", "Int" ); $_[0]{"xyzzy"} = $_[1]; $_[0]; } : ( $_[0]{"xyzzy"} ) }


BEGIN {
    require MooseInteg::BaseClassRole;
    
    our %DOES = ( "MooseInteg::BaseClass" => 1, "MooseInteg::BaseClassRole" => 1 );
}
